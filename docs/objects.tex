...
\newcommand{\ABSTRACT}{
This document summarizes interfaces that are instrumental for
the interaction with Clouds, Containers, and HPC systems to manage
virtual clusters to support the Big Data Reference Architecture. The
...
Big Data Reference Architecture (NBDRA) which is introduced in Volume
6.}

\newcommand{\KEYWORDS}{NIST Big Data Reference Architecture;
  Interfaces, REST}
...
approach that we need to implement the refernce architecture depicted
...
\subsubsection{Big Data Provider Framework Interface Requirements}\label{S:r-provider}
...
the document it would introduce too complex of a notation framework.
This would result in (a) a considerable increase in length of this
document (b) a more complex framework reducing participation and (c) a
more complex framework for developing a reference implementation. Thus
we have decided in this version of the document to introduce a design
concept by example that is used to automatically create a schema as
...
and a schema is generated from it. The schema is added to the
Appendix~\ref{s:schema} of the document.

While focusing first on examples it allows us to speed
...
generated. A hyperlink is introduced in the Object specification than
when clicking on the \verb|</>| icon you will be redirected to the
specification in github.


The resulting schema object follows the Cerberus \cite{www-cerberus}
specification and looks for our object as follows:
...
As mentioned before, the Appendix~\ref{s:schema} will list the schema
...
The interfaces are summarized in Figure \ref{F:interfaces}.
