...
providers. They can be either local data providers, data contributed by a user. or
distributed data providers that refer to data on the Internet. 
Currently, the focus is on an elementary set of abstractions related to data
providers that offer the ability to utilize variables, files, virtual data
directories, data streams, and data filters. The following definitions are 
used to construct the abstractions.

\begin{description}
\item[Variables] are used to hold specific contents that are associated
  in programming language as variables. A variable has a name, value,
  and type.

\item[Defaults] are special type of variables that allow adding of a
  context. Defaults can be created for different contexts.
...
\item[Virtual Directories] are collections
  of endpoints to files. Files in a virtual directory may be located
  on different resources. For this initial purpose, the distinction
  between virtual and non-virtual directories is non-essential and the
  focus will be on abstracting all directories to be virtual. 
  Therefore, the files coudl be physically hosted on different
...
\item[Streams] are services that offer the consumer a flow of
...
\item[Batch Filters] operate on streams and on files, working in
  the background and delivering files as output. In contrast to
  streams, batch filters process the data set and return an output after all
  operations have been applied.

\item[Indexed Stores] are storage systems for objects that can be
  accessed by an index for each object. Search and filter functions
  are integrated into indexed stores to allow identification of objects.

\item[Databases] refer to traditional relational databases 
  but also to NoSQL databases.

\item[Collections] are an agglomeration of any type of data.

\item[Replicas] are duplication of data objects created to avoid
