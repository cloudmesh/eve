...
based on the associated context. For example, for a default could be the image
...
It is important to provide a mechanism to trace the
...
A virtual directory is a collection of files or replicas of the files. A virtual directory can contain a
...
scheduled based on resource permission, are called batch jobs. Batch jobs are
used to minimize human interaction and allow the submission and
...
today's shifting Internet of things environment towards clouds and containers. This is
...
performance advantages. Disadvantages include the limited and preinstalled
...
The \textit{Map/Reduce} deployment has as inputs parameters defining
the applied function and the input data. Both function and data
objects define a {\it source} parameter, which specfies the location it
is retrieved from. For instance, the ``file://'' Uniform Resource Identifier (URI)
indicates sending a directory structure from the local file system and the ``ftp://''
indicates that the data should be fetched from a File Transfer Protocol (FTP) resource. It is
the framework's responsibility to materialize an instantiation of the
desired environment along with the function and data.

\codeFromJson{json}{mapreduce.json}{Mapreduce}{o:mapreduce}

Additional parameters include the {\it fault\_tolerant} and {\it backend}
parameters. The former flag indicates if the \textit{Map/Reduce}
...
name nodes using Zookeeper. The {\it backend} parameter accepts an
object describing the system providing the \textit{Map/Reduce}
...
generated results. For input parameters, the {\it buildInputs} and
{\it systemBuildInputs}, respectively, describe the objects that should
be evaluated and system packages that should be present before this
function can be installed. The {\it eval} attribute describes how to
apply this function to its input data. Parameters affecting the
evaluation of the function may be passed in as the {\it args} attribute.
The results of the function application can be accessed via the
{\it outputs} object, which is a mapping from arbitrary keys
...
One example function is the {\it NoOp} function shown in
...
the identity element is the empty mapping while for lists the identity element is the empty list.
...
A \textit{Hadoop} definition defines which \textit{deployer} to use,
the \textit{parameters} of the deployment, and the system packages
\textit{required}. For each requirement, it could have attributes such
as the library origin, version, and other attributes (see Object \ref{o:hadoop})
...
Big Data applications and systems use a considerable amount of
resources, which requires an accounting system, either on the
...
existing accounting frameworks may not be present to deal with
this issue. For example, potential use of multiple accounting
systems with different scales of accuracy for information feedback
rates could occur. If the existing accounting system informs the user
of limited resources only hours after she has started a job, a significant
risk could arise because charging is started immediately. While making access to
Big Data infrastructure and services simpler, the user or
...
account for a user. The services that are then invoked by the user can
consume the resource and are charged accordingly.
...
\item An accompanying message for the HTTP response code, and
